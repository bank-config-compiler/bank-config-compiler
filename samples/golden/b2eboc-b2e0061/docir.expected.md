...
|---|---|---|---|---|---|---|---|---|---|
| 1 |  | `bocb2e` | [1..1] | Object | Y | BOCB2E XML 顶层块 |  | 一个 HTTP 请求或响应只能且必须包含一个 BOCB2E XML block |  |
...
| 1.5.1 |  | 　　`termid` | [1..1] | String | Y | 代表一台企业前置机 | E 开头 + 前置机 IP 地址，各段补零，无小数点 12 位 | 检查终端号是否维护 |  |
| 1.5.2 |  | 　　`trnid` | [1..1] | String | Y | 客户端产生的报文编号 | 字母数字串 0-12 位 |  |  |
| 1.5.3 |  | 　　`custid` | [1..1] | String | Y | 企业在中行网银系统的客户编码 | 数码 1-10 位 |  |  |
| 1.5.4 |  | 　　`cusopr` | [1..1] | String | Y | 企业操作员代码 | 字母数字标点串 1-20 位 | 检查操作员是否存在 |  |
| 1.5.5 |  | 　　`trncod` | [1..1] | String | Y | 交易代码 | b2e 开头加 4 位数字 | 检查交易是否存在，与报文体是否一致 |  |
| 1.5.6 |  | 　　`token` | [0..1] | String | N | 交易验证标识，签到时生成、签退时注销 | Base64 字符串 0-64 位 | 检查令牌是否正确 | 请求示例可缺省，响应示例出现；方向差异需确认。 |
| 1.6 |  | 　`trans` | [1..1] | Object | Y | 交易数据块 |  | b2e0061 交易 wrapper 挂载于该节点下 |  |
...
| 2.3.1 |  | 　　`insid` | [1..1] | String | Y | 指令ID；本转账指令在客户端的唯一标识，建议企业按时间顺序生成且不超过12位 | 非空字符串；长度1-32 | 客户号下不能重复；不支持中文 |  |
| 2.3.2 |  | 　　`obssid` | [0..1] | String | N | 网银交易流水号 | 可空，若不为空只为数字且长度1-30位 | 交易类型为2时有效且非空；流水对应交易必须存在且为该客户下退回类公转私交易 |  |
| 2.3.3 |  | 　　`fractn` | [1..1] | Object | Y | 付款人账户 |  |  | 必填性由必填子字段推导。 |
| 2.3.3.1 |  | 　　　`fribkn` | [0..1] | String | N | 付款行联行号 | 可空数码5位或12位 | 可空，数码5位；联行号有对应的省行联行号 | 前置机和平台长度口径不一致。 |
| 2.3.3.2 |  | 　　　`actacn` | [1..1] | String | Y | 付款账号 | 非空字符串；1-35位 | 非空字符1-20位；账户已维护，操作员有权限；不支持中文 | 前置机和平台长度口径不一致。 |
| 2.3.3.3 |  | 　　　`actnam` | [0..1] | String | N | 付款人名称 | 可空字符串；长度0-70 | 虚拟账号时上送主账号名称 |  |
| 2.3.4 |  | 　　`toactn` | [1..1] | Object | Y | 收款人账户信息 |  |  | 必填性由必填子字段推导。 |
| 2.3.4.1 |  | 　　　`toibkn` | [0..1] | String | N | 收款行联行号 | 数码；长度0-12 | 可空；5位或12位数字；中行账户支持上送12位 CNAPS 号 |  |
| 2.3.4.2 |  | 　　　`acttyp` | [0..1] | String | N | 收款账户类型 | 数码；长度0-3 | 可空，默认借记卡；119=借记卡，101=普活活期，188=活一本，103=信用卡；上送但无业务校验 |  |
| 2.3.4.3 |  | 　　　`actacn` | [1..1] | String | Y | 收款账号 | 非空字符串；长度1-35 | 中行账户且长度18位时必须上送收款行联行号；不支持中文 |  |
| 2.3.4.4 |  | 　　　`toname` | [1..1] | String | Y | 收款人名称 | 非空字符串；长度1-70 | 一个汉字等于一个字符 |  |
| 2.3.4.5 |  | 　　　`tobknm` | [0..1] | String | C | 收款人开户行名称 | 可空字符串；0-70位 | 当收款行联行号为空时必填；系统可根据联行号或 CNAPS 号补全 | 条件必填。 |
| 2.3.4.6 |  | 　　　`toaddr` | [0..1] | String | N | 收款人地址 | 可空字符串；0-70位 | 一个汉字占2个字符 |  |
| 2.3.4.7 |  | 　　　`email` | [0..1] | String | N | 收款人电子邮件地址 |  | 可空；非空时包含 @，3-80 位 |  |
| 2.3.5 |  | 　　`trnamt` | [1..1] | Decimal | Y | 转账金额 | 非空正数字；长度(22,2) | 非空正数15,2位；根据交易币种辅币位数检查金额格式；不超过转账限额 | 前置机和平台长度口径不一致。 |
| 2.3.6 |  | 　　`trncur` | [1..1] | String | Y | 转账货币 | 非空3位大写字母、数字 | 只支持001或者CNY，仅人民币 |  |
| 2.3.7 |  | 　　`priolv` | [1..1] | String | Y | 银行处理优先级 | 非空字母数字字符串1-2位 | 枚举：0=普通，1=加急 |  |
| 2.3.8 |  | 　　`cuspriolv` | [1..1] | String | Y | 客户处理优先级 | 非空字母数字字符串1-2位 | 枚举：0=普通，1=快速 |  |
| 2.3.9 |  | 　　`furinfo` | [0..1] | String | N | 用途 | 字符串，长度0-200，不支持 `\|` | 可空；一个汉字占2个字符 |  |
| 2.3.10 |  | 　　`trfdate` | [1..1] | Date | Y | 要求的转账日期；YYYYMMDD | 日期；YYYYMMDD | 系统当前日期含当日之后的一个月内 |  |
| 2.3.11 |  | 　　`trftime` | [0..1] | String | N | 要求的转账时间；HHMMSS | 可空，时间 HHMMSS | 当日时为当前时间后整点；当日后一个月内为整点；HH0000（000000-230000）默认为000000 | 是否只允许整点需确认。 |
| 2.3.12 |  | 　　`comacn` | [0..1] | String | N | 支付费用账号 | 可空字符串；0-35位 | 非空则1-20位且账户已维护；为空使用付款账户代替 | 前置机和平台长度口径不一致。 |
| 2.3.13 |  | 　　`bocflag` | [1..1] | String | Y | 收款人是否中行账号 | 非空字母数字1-2位 | 枚举：1=中行，0=他行 |  |
...
| Index | Or | Message Item | Mult. | Type | Required | 说明 | 前置机校验点/格式 | 接口平台校验点 | Review |
|---|---|---|---|---|---|---|---|---|---|
| 3 |  | `trn-b2e0061-rs` | [1..1] | Object | Y | 对应请求的响应 |  |  | 交易包装节点。 |
...
| 3.2 |  | 　`b2e0061-rs` | [0..1000] | Object | N | 每条转账指令响应内容 |  |  | 原文明确 `(0..1000)`。 |
| 3.2.1 |  | 　　`status` | [1..1] | Object | Y | 每条划账指令处理状态 |  |  | 每条响应内状态。 |
