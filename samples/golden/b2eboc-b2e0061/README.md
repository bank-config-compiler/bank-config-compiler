...
这些 expected artifacts 是 ADR-0008 修订前形成的“审查前”Golden，本批次保持原样。Human 与银行线下确认 ASSEMBLY、PARSE 两个方向的 Final `xmlEncoding` 均为 `UTF-8`；该结论将在后续 P0-T3 SchemaIR fixture 中落实。以后银行文档若出现不同 encoding，Validator 必须产生 Warning 并阻止 Final，直到 Human Review 给出新结论。银行事实投影、Standard 镜像和结构绑定同样留给后续 fixture；本目录只通过本 README 指向新契约。
