# b2e0061 Review Golden Review Notes / 人工评审说明

Status: REVIEW_GOLDEN。用于冻结 expected review output；不是 final business answer，也不是 runtime final contract。

Review input: 本文件吸收当前人工评审反馈；评审过程中的 scratch 文件不作为 repository artifact 管理。下列待确认项是本 Review Golden sample 的 expected review notes，后续生成器应稳定复现这些人工确认入口。
...
3. 请求报文 `<b2e0061-rq>` 原文只写“不超过1000笔”。当前 Review Golden sample 保留 `occurs: "0..1000"`、`multiple: true`，同时将 `required=true` 标为待确认；需要确认请求最小出现次数是否应为 `1`。
...
1. 本 Review Golden sample 使用 `samples/golden/b2eboc-b2e0061/raw-doc.md` 作为受控样例 source。若源文档本身错误，应先修正 raw-doc，再进入 DocIR / SchemaIR 转换。
...
5. 每次 DocIR / SchemaIR draft 生成都应产出类似本文件的 `review-notes.md`；进入 Review Golden sample 后，对应 expected artifact 固定为 `review-notes.expected.md`。
