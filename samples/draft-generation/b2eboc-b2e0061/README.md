...
候选来源均为仓库内已有的 review artifacts：DocIR 来自 Review Golden；SchemaIR 是历史 Draft 按 `draft-prompt/v10` 精确 semantic candidate shape 投影后的受控输入；InterfaceStandardIR 和 InterfaceTemplateIR 来自对应 Human Review gate 前的历史 Draft。fixture manifest 显式绑定 task/interface、Schema identity 和 Standard identity；运行时仍会经过当前 materializer、Validator 与 lineage publication。它们用于证明 provider boundary、确定性投影和持久化路径，不新增银行业务事实。

`deng` 已批准九列 DocIR candidate hash `sha256:369b7fb62b1447226d404fe1e67e980933c8d1538a1635f7ef6b72454e16207b`。`docir-final.md` 是 candidate 的逐字节冻结副本，`docir-final-review.md` 记录具名批准、时间、格式迁移、Object Required=N/A、Conditions 条件分支边界、保留不确定性的结论和失效边界。

## Trust boundary

- SchemaIR provider output 只包含 v10 白名单语义，不携带 lifecycle、identity 或可派生结构字段；materializer 重建的公开 JSON Draft，以及 Standard/Template provider output，必须保持 `status=DRAFT`、`review.status=PENDING`。
