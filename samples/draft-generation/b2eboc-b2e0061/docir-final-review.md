...
- Approved Draft bytes hash：`sha256:31d7fc002ccc2b840f401206f54665e36771f2bb5502d480566defdff9ac7585`
- Approved Draft size：`12228` bytes
...
2. `version=120` 与示例 `100`、`@locale` 与 observed `@lang`、请求重复节点出现次数、两组约束及 `rspmsg`/`errmsg` 等冲突继续作为原文事实和不确定性保留。
3. `ceitinfo` 是否可配置、目标系统采用哪组约束以及正式配置策略属于后续 SchemaIR/Standard/Template Review，不在 DocIR 中提前决定。
4. 本次批准确认的是 raw doc 表达的忠实性，不等于批准上述未决业务结论。
