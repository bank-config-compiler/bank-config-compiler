# b2e0061 DocIR Draft Review Notes / 人工评审说明

Status: `PENDING`。该记录绑定生成器补充的准确 candidate hash；不是 Final DocIR 或已批准的业务结论。

Review input: 本文件复用既有 Review Golden 中的人工确认入口，不新增银行事实。下列项目在 reviewer 明确确认准确 candidate hash 前保持待确认。
...
1. 本候选使用 `samples/golden/b2eboc-b2e0061/raw-doc.md` 作为受控样例 source。若源文档本身错误，应先修正 raw-doc，再进入 DocIR / SchemaIR 转换。
