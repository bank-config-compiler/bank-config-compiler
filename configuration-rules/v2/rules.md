...
ASSEMBLY field config 在 `standardTarget` 中显式保存 `standardFieldRef` 与 `standardProjection.required/length/dataType`，projection 必须与绑定 Final Standard 完全相同。

PARSE 的 Standard 是表达式 source，而不是 field config 的单一 target。每个 FIELD_REF 直接保存自己的 `standardFieldRef`；`COLLECTION_ITEM` 通过 `standardSource.standardFieldRef` 保存集合结构来源。Validator 从 Template 精确绑定的 Final Standard 确定性解析 required、length 和 dataType，不在 PARSE field config 或表达式节点重复保存 projection。复合表达式可以引用多个 Standard Field，纯 literal/EMPTY 表达式也可以没有 Standard source；不得选择或制造一个顶层“主” Standard source。
...
- ASSEMBLY `standardTarget.standardProjection` 的 required、length 或 dataType 与绑定 Standard 不一致；
- PARSE FIELD_REF 或 collection `standardSource` 引用的 Standard Field 不存在、方向错误或无法从精确绑定的 Final Standard 解析；
