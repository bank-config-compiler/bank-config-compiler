...
`configuration-rules/v1` 与只修订方向相关 Standard projection 的 v2 均已发布并冻结。SchemaIR v2、InterfaceStandardIR、InterfaceTemplateIR、Configuration Workbook、双方向 Golden 与匹配 validation results 均已实现并冻结；P0-T3 已完成。P0-T4 的 provider-neutral runtime、六个受控 response、四类 CLI 和准确 hash 的 reviewed Final DocIR 已冻结；完整受控回归显式装载已审核 Final fixtures，双方向 Workbook 结构化内容均与 Golden 一致。P0-T5 的真实 provider adapter、显式配置、Prompt/context 边界与离线测试候选已经实现。`docir-001` 至 `docir-010` 验证并收口了最小门禁、流式长响应、结构化 shape 与诊断问题；`draft-prompt/v7` 的 `docir-011` 消除了 envelope 歧义，但完整流只包含到不完整的 `assembly` 且缺少 `parse`，被通用机械门禁拒绝。ADR-0014 的 `draft-prompt/v8` 有界分段实现与 attempt v2 evidence 已通过离线回归，尚未发起新真实 attempt。Golden 不参与真实候选自动语义判定。其余真实 Draft、逐层 Final validation 与双方向 Workbook 证据仍未完成，Phase0-PoC 为 In Progress。Final IR 必须精确引用适用的 `RELEASED` 规则版本。
