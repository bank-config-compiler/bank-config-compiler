...
`configuration-rules/v1` 已发布并冻结；SchemaIR v2 runtime 与已评审 Final fixture 已实现，两个目标配置 IR 的 wire contract、Validator、golden fixture 和 Workbook Generator 仍处于 P0-T3 后续批次。Final IR 必须精确引用适用的 `RELEASED` 规则版本；正式导出只能作为经治理的目标配置证据，不能直接代替 IR 或 Generator 输入。
