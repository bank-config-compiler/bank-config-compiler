...
- Commit 8A4 的原分支提交为 `25e03ce`，相关实现与验证通过 PR #19 的主线 squash commit `dd8d0d5` 合入；231 项 warnings-as-errors pytest、build、docs-sync、diff/BOM/secret 检查均通过。
...
- `docir-019` 证明有界分段可以产生连续的基本可用内容，同时也暴露出严格局部 invariant 的模型方差与全量重跑成本。ADR-0015 已决定不采用 LLM correction，而是由代码物化机械字段：transport/incomplete 或不可物化结构仍按本 ADR fail-fast；完整可物化候选即使校验失败也可以作为 Invalid Draft 原子发布。新的 attempt 仍不得自动 retry/resume 或复用旧 prefix。
