...
P0-T3 trusted chain 与 P0-T4 deterministic Draft-to-Workbook closure 已完成。ADR-0015 已接受 candidate → deterministic materialization → Invalid/Reviewable Draft → Human Gate 的六类 Draft 共同策略，ADR-0016 将 DocIR Type 和非重复 Multiplicity 收归代码规范化，并保留 Required 缺证据的 Human Gate。`docir-020` 是未发布 Draft 的历史失败 attempt；`docir-021` 已真实发布 49 fields、75 ERROR 的 Invalid Draft并完成重新校验，二者均不得复用。v2 materializer 对 `docir-021` candidate 的只读离线重放为 49 fields、15 个 Required ERROR，只证明新规则的收敛效果，不改写 attempt、generation lineage 或工作 Draft。
...
3. 增加内部 `docir-semantic-candidate/v1`，由代码根据有序树分配 index、固定 wire 和 Review marker。
...
- [x] `draft-prompt/v14` 与 `docir-semantic-materializer/v2` 规范化 Type/非重复 Mult.，同时保持 Required 缺证据为 ERROR。
...
- P0-T5 真实 Final DocIR 仍需要用户单独授权唯一 attempt `docir-022`，并对新 Draft 的 Required、重复范围、`locale/lang` 等来源决策执行 Human 修改、重验和 approval；`docir-020`、`docir-021` 与离线重放均不得复用为 generation lineage。
