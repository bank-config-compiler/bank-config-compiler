...
In Progress. P0-T3 trusted chain 与 P0-T4 deterministic Draft-to-Workbook closure 均已完成；P0-T5 adapter、严格流式传输、内部结构化 DocIR extraction、确定性 renderer/review notes，以及 ADR-0014 的有界分段与 attempt v2 evidence 均已完成离线实现。`docir-014/015` 证明空 wire 值 Review invariant 必须覆盖两个 full-field 阶段；v11 的 `docir-016` 已满足该规则但属性 index 混入 XML 名。v12 已明确纯数字层级 regex 与 name/index 隔离，尚待新 attempt 从头验证。当前没有通过 Human Review 的真实 DocIR candidate，Phase0-PoC 最终门禁未通过。
...
| P0-T5：真实 LLM Draft-to-Workbook 验证 | In Progress | P0-T4、用户批准的 OpenAI-compatible Chat API provider 与测试样例 | adapter、分段 DocIR、attempt v2 evidence 和离线回归已实现；`docir-016` 通过 Review 标记规则但暴露纯数字 index prompt 缺口；v12 已修正，尚无通过 Human Review 的真实 DocIR candidate | 提交 v12 后，以新 attempt ID 从第一段验证；不得 resume 任何失败 attempt。 |
...
| `cli.py` | `ingest`、`check --profile raw|phase0`、`generate-workbook` 与 `generate-draft docir|schemair|standard|template`；支持 fixture/真实 provider，DocIR 可显式设置正整数字段 batch size | 无离线实现差距；真实 candidate 与后续链路证据尚缺 | P0-T5 Commit 8A3 / 8B |
...
| Draft generators | provider-neutral runtime、真实 Chat adapter、DocIR 有界分段、attempt v2 evidence、六个受控 response、CLI、Draft publication、reviewed Final DocIR 与完整 closure regression 已实现 | 没有新的真实 DocIR、逐层 Review/Final validation 和双方向 Workbook 验证证据 | P0-T5 Commit 8A3 / 8B |
...
3. **v7 单响应完整性问题已形成有界分段实现，真实分段验证继续收紧 prompt。** `docir-012/013` 暴露 shape 与职责问题。v9 的 `docir-014` 已使 12-node Envelope 与 27/10-field 紧凑 outline 通过门禁，但首个 ASSEMBLY detail 没有为空的 wire 值写入 Review 标记；`docir-015` 又在边界正确的 Envelope 随机复现相同缺口。v11 统一两个 full-field 阶段后，`docir-016` 已满足 Review 标记并保持 12-node `trans` 边界，但三个属性 index 使用 `1.@name`。v12 已加入精确纯数字层级 regex、有效/无效示例及 name/index 隔离；下一 attempt 必须从第一段重新执行。
...
- 六个真实 Draft artifact 均由 adapter 发起，并在同一 Draft contract 下发布 `DRAFT/PENDING` artifact；DocIR 的全部 subcall、extraction、确定性 Markdown wire、最小结构检查和 Human Review rubric 均通过，三个 JSON Draft 均为 0 ERROR 且 `finalEligible=false`。
...
#### 已完成 Commit 8A1：真实 OpenAI-compatible provider 与严格流式传输
...
#### 已完成 Commit 8A2：结构化 DocIR extraction 与确定性 renderer
...
- Completion signal：结构化非法输入 fail closed；固定章节/表头/Index/U+3000/值域和 Review Notes 顺序由离线测试确定性保证；fixture regression 不变。真实 candidate 属于后续验收，不阻塞已实现的确定性边界。
...
- Scope：执行六个真实 Draft artifact（DocIR 内含多个 subcall），逐层 Human Review/Final validation，生成双方向 Workbook，并保存经过数据保留审查的最小证据摘要；不把敏感 artifact 自动加入 Git。
...
**状态：IN PROGRESS。P0-T3 与 P0-T4 的完成标志已由实现和自动化 regression 覆盖；P0-T5 adapter、严格流式调用、direct extraction、确定性 renderer/review notes、DocIR 有界分段与 attempt v2 evidence 已完成离线实现。职责隔离与空 wire 值 Review invariant 已分别由 `docir-014` 和 `docir-016` 正向证明；`docir-016` 新暴露 index/name 混淆并已由 v12 精确收紧。失败 evidence 已保留且未发布部分 Draft。当前没有可冻结真实 DocIR，Human Review-to-Workbook 验收仍未完成；下一步以全新 attempt 从第一段验证。**
