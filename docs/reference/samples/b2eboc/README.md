...
`b2e0061.md` 已作为 Phase0 raw doc 输入，并在 `samples/golden/b2eboc-b2e0061/` 中形成 DocIR / SchemaIR Review Golden sample。本 reference 目录本身不是 golden sample；正式导出和字段清单已经进入 `configuration-rules/v1` Draft 的证据链，InterfaceStandardIR、InterfaceTemplateIR、对应 Validator 和 workbook assertions 仍需在 P0-T3 中实现和确认。
