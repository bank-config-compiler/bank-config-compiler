...
- P0-T5 仍要求真实完成六个 Draft artifact、每层独立 Human Review/Final validation 与双方向 Workbook 验证；adapter 与离线自动化已经实现。`docir-001` 至 `docir-010` 依次暴露最小门禁、直出 Markdown、长响应 transport、外层 envelope 和诊断问题；`draft-prompt/v7` 的 `docir-011` 返回了正常结束但缺少 `parse` 的单响应 extraction。ADR-0014 随后实现 attempt 原子化的有界分段和 v2 evidence；首次真实分段 attempt `docir-012` 在 `interface-envelope` 返回完整 JSON，但把 prompt 未明确 shape 的 `sourceContext` 生成为 object，被要求非空字符串数组的机械门禁拒绝并立即停止。prompt 已与既有 Validator 对齐，下一次验证必须以新 attempt 从头执行。当前仍无可冻结真实 DocIR，Phase0-PoC 尚未完成。
