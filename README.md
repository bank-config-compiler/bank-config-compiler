...
- `openai-chat` 使用官方 OpenAI Python SDK 的最小流式 Chat Completions 子集与 JSON object response mode。DocIR 的 `draft-prompt/v16` 只让模型提出有序 XML semantic tree、来源明确的重复 Object、标量类型和标量 Required 语义；代码校验唯一根、父子结构、顺序和 detail coverage，再以 `docir-semantic-materializer/v4` 分配 index、从树派生 `Object/String`、将非重复 `Mult.` 规范化为空并渲染九列 Markdown wire。Object Required 固定为空（N/A），不从叶子反推；标量缺失使用明确的 Required Review marker。代码只对 candidate/Draft 中已有标量 Required 证据做一致性门禁，不自动填写 `Y/N/C`；Review Notes 生成不发起额外 LLM 调用。SchemaIR、Standard、Template 的 `draft-prompt/v9` 同样只请求语义 candidate；SchemaIR Object 出现性由 candidate/Human 独立确认，其余稳定身份和可从 Final 上游唯一推导的结构字段由 materializer 锁定或重算。原始 response/candidate 仅保存在 Git-ignored attempt evidence 中，不是公开 IR 或下游输入。
- ADR-0015 已接受六类 Draft 的统一可信链，ADR-0016/0017 收敛 DocIR Type、Multiplicity、九列格式与 Required 证据门禁，ADR-0018 明确 Object Required=N/A。`docir-021` 与 `docir-022` 的 provider-attempt evidence 均保持 immutable；`docir-022` 当前 Human working Draft 已迁移并重新校验，仍需完成标量 Required 修订、重验和 approval。逐层 Human approval 和双方向 Workbook 验收尚未完成，因此 Phase0-PoC 仍为 `In Progress`。
