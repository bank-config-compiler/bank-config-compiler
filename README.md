...
- `openai-chat` 使用官方 OpenAI Python SDK 的最小流式 Chat Completions 子集与 JSON object response mode。DocIR 的 `draft-prompt/v17` 只让模型提出有序 XML semantic tree、来源明确的重复 Object、标量类型、标量 Required 语义和明确条件分支；代码校验唯一根、父子结构、顺序和 detail coverage，再以 `docir-semantic-materializer/v4` 分配 index、从树派生 `Object/String`、将非重复 `Mult.` 规范化为空并渲染九列 Markdown wire。Object Required 固定为空（N/A），不从叶子反推；标量缺失使用明确的 Required Review marker。最大笔数、格式、唯一性等普通字段约束不得进入 Conditions；不符合条件分支形式的条目会阻塞。代码只对 candidate/Draft 中已有语义证据做确定性门禁，不自动填写 `Y/N/C` 或重写 Conditions；Review Notes 生成不发起额外 LLM 调用。SchemaIR、Standard、Template 的 `draft-prompt/v9` 同样只请求语义 candidate；SchemaIR Object 出现性由 candidate/Human 独立确认，其余稳定身份和可从 Final 上游唯一推导的结构字段由 materializer 锁定或重算。原始 response/candidate 仅保存在 Git-ignored attempt evidence 中，不是公开 IR 或下游输入。
...
P0-T3、P0-T4 与 P0-T5 均为 `Done`。P0-T6 的下游五类 IR materializer/Human Gate 已完成离线实现，现已由真实 Final DocIR 解锁；下一步是生成、校验并人工批准真实 SchemaIR。Phase0-PoC 当前仍为 `In Progress`。详细状态见 [Phase0-PoC 执行计划](docs/planning/00-phase0-poc-plan.md)。
