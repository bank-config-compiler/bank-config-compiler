...
- `openai-chat` 使用官方 OpenAI Python SDK 的最小流式 Chat Completions 子集与 JSON object response mode。DocIR 的 `draft-prompt/v17` 只让模型提出有序 XML semantic tree、来源明确的重复 Object、标量类型、标量 Required 语义和明确条件分支；代码校验唯一根、父子结构、顺序和 detail coverage，再以 `docir-semantic-materializer/v4` 分配 index、从树派生 `Object/String`、将非重复 `Mult.` 规范化为空并渲染九列 Markdown wire。Object Required 固定为空（N/A），不从叶子反推；标量缺失使用明确的 Required Review marker。最大笔数、格式、唯一性等普通字段约束不得进入 Conditions；不符合条件分支形式的条目会阻塞。代码只对 candidate/Draft 中已有语义证据做确定性门禁，不自动填写 `Y/N/C` 或重写 Conditions；Review Notes 生成不发起额外 LLM 调用。SchemaIR 使用 `draft-prompt/v10` 的精确 semantic candidate shape；materializer 对 top/envelope/message/field 和嵌套 evidence/condition 属性执行白名单门禁，再从候选语义与 Final DocIR 确定性重建公开 SchemaIR。未知或缺失 candidate 属性属于 hard failure，不发布 Invalid Draft。Standard、Template 继续使用 `draft-prompt/v9`；稳定身份和可从 Final 上游唯一推导的结构字段由各自 materializer 锁定或重算。原始 response/candidate 仅保存在 Git-ignored attempt evidence 中，不是公开 IR 或下游输入。
- ADR-0015 已接受六类 Draft 的统一可信链，ADR-0016/0017 收敛 DocIR Type、Multiplicity、九列格式与 Required 证据门禁，ADR-0018 明确 Object Required=N/A，ADR-0019 将 Conditions 收窄为 raw doc 明确条件分支。`docir-021` 与 `docir-022` 的 provider-attempt evidence 均保持 immutable；`docir-022` 已由 `deng` 对准确 bytes 完成 Human approval，真实 Final DocIR hash 为 `sha256:180dadcc10fea5cf364c72e7b36d6d36aad3bfc24d3edd172138b24869042ae6`，Validation Result 为零 ERROR、两个已审阅的非阻塞跨字段 WARNING。P0-T5 已完成；五份下游真实 Final 和双方向 Workbook 验收尚未完成，因此 Phase0-PoC 仍为 `In Progress`。
...
P0-T3、P0-T4 与 P0-T5 均为 `Done`。P0-T6 的下游五类 IR materializer/Human Gate 已完成离线实现，现已由真实 Final DocIR 解锁。`schemair-001` 与 `schemair-002` 已消费且不得复用；后者保留了暴露 candidate contract 缺口的 Invalid Draft/evidence。该缺口已由 SchemaIR `draft-prompt/v10` 与严格 materialization 门禁离线修复；下一执行 Gate 必须使用新的 `schemair-003`，再由 Human 审查和批准准确 Draft。Phase0-PoC 当前仍为 `In Progress`。详细状态见 [Phase0-PoC 执行计划](docs/planning/00-phase0-poc-plan.md)。
