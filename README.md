...
- P0-T5 仍要求真实完成六个 Draft artifact、每层独立 Human Review/Final validation 与双方向 Workbook 验证；adapter 与离线自动化已经实现。`docir-001` 至 `docir-010` 依次暴露最小门禁、直出 Markdown、长响应 transport、外层 envelope 和诊断问题；`draft-prompt/v7` 的 `docir-011` 返回了正常结束但缺少 `parse` 的单响应 extraction。ADR-0014 随后实现 attempt 原子化的有界分段和 v2 evidence。`docir-012/013` 分别暴露 shape 与职责问题；`docir-014/015` 证明空 wire 值 Review invariant 必须覆盖两个 full-field 阶段；`docir-016` 随后暴露 index/name 混淆。v12 的 `docir-017` 首段已满足 12-node `trans` 边界、纯数字 index、十属性和空值 Review 全部门禁，但联合 outline 在无可校验响应的情况下发生 `ReadTimeout`，attempt 原子停止。离线逆向与测试已确认原 timeout 只形成逐次 I/O 上限，不能限制持续产出非内容分块的总墙钟时间；物理 subcall 绝对期限已补齐。当前仍无可冻结真实 DocIR；再次真实调用必须另获授权，并以新 attempt ID 从第一段开始，Phase0-PoC 尚未完成。
