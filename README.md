...
- P0-T5 仍要求真实完成六个 Draft artifact、每层独立 Human Review/Final validation 与双方向 Workbook 验证；adapter 与离线自动化已经实现。`docir-001` 至 `docir-010` 依次暴露最小门禁、直出 Markdown、长响应 transport、外层 envelope 和诊断问题；`draft-prompt/v7` 的 `docir-011` 返回了正常结束但缺少 `parse` 的单响应 extraction。ADR-0014 随后实现 attempt 原子化的有界分段和 v2 evidence。`docir-012/013` 分别暴露 shape 与职责问题；`docir-014/015` 证明空 wire 值 Review invariant 必须覆盖两个 full-field 阶段。v11 的 `docir-016` 已满足该标记规则并保持 12-node `trans` 边界，但把三个属性 index 写成 `1.@name` 而被纯数字层级门禁拒绝。v12 已加入精确 regex、有效/无效示例和 XML 名隔离规则；下一次验证仍须使用新 attempt 从头执行。当前仍无可冻结真实 DocIR，Phase0-PoC 尚未完成。
